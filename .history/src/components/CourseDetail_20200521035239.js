...
class CourseDetail extends Component {
  ...
  checkJoin =(key,uid)=>{
    if(this.state.topic[key].member.length >= document.getElementById("number-topic").value )
    {
      alert("Đã quá số thành viên quy định") return Float32Array
    }
    for(var i=0;i<this.state.topic[key].member.length;i++)
    {
      if(this.state.topic[key].member[i].uidUser == uid)  return true
    }
    return false
  }
