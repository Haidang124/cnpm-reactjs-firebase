...
class CourseDetail extends Component {
  ...
  checkJoin =(key,uid)=>{
    this.state.topic[key].member.map((item,index)=>{
      if(item.uidUser == uid) return true
    })
    return false
  }
    ...
    this.state.topic.map((item, key) => {
      if (item.keyTopic == keyTopic) {
        if(this.checkJoin(key,uidUser)==false)
        {
          ...
          alert("Bạn đã tham gia từ trước nên không thể tham gia lại")
        }
      }
    })
  } 
              ...
              <tbody>
                {this.state.topic.map((item, key) => (
                  <tr>
                    ...
                    <th>
                      <ButtonGroup>
                        <Button color="link" onClick={()=>this.joinTopic(item.keyTopic)}>Join</Button>
