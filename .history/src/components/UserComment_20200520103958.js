...
class UserComment extends Component {
  ...
  componentDidMount() {
    ...
    //           if (value.keyPost == this.props.keyComment) {
    //             this.setState({
    //               currentComment: doc.data().post[key].comment[key1],
    //               fullDataComment: doc.data().post[key].comment,
    //             });
    //           }
    //         });
    //       }
    ...
    db.collection("courses")
      ...
      .onSnapshot((doc) => {
        doc.data().post[this.state.indexPost].comment.map((item, key) => {
          if (item.keyComment == this.props.keyComment) {
            this.setState({
              currentComment: doc.data().post[this.s],
              fullDataComment: doc.data().post,
            });
          }
        });
      });
