...
class Profile extends Component {
  ...
  if(this.state.avatar === '') {
    console.error(`not an image, the image file is a ${typeof(this.state.avatar)}`)
  }
  const uploadTask = storage.ref(`/images/${this.state.avatar.name}`).put(this.state.avatar)
  ...
  (snapShot) => {
    ...
    storage.ref('images').child(this.state.avatar.name).getDownloadURL()
     .then(fireBaseUrl => {
