...
class ProjectTask extends Component {
  ...
  startTask=(executor,index)=>{
    if(executor == store.getState().userAuth.uid){
        this.startModal();
        // alert(index)
        if(this.state.beganStart == true) this.startTaskNow(index)
        
        var startnow = document.getElementById("start-now")
        // startnow.onClick = this.startTaskNow(index)
        startnow.onclick = function(e) { 
          this.startTaskNow(index)
      };
    }
    else alert("Bạn không thể bắt đầu công việc của người khác")
  }
  startTaskNow=(index)=>{
    this.state.fulldataTopic[this.state.indexTopic].alltaskProject[index].status="Process";
    var newdata = {
    ...
    return (
      <div className="project-task">
                  ...
                  <th>
                    <select className="input-modal" id="executor-task">
                      ...
                      {this.state.member.map((item, key) => (
                        <option value={item.uidUser}>{item.user}</option>
                      ))}
                    </select>
                  </th>
                  <th>
        ...
        <Modal isOpen={this.state.isStartModal}>
          ...
          <ModalFooter>
            <Button color="primary" id="start-now">Start Now</Button>
