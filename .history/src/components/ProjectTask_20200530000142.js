    ...
    return (
      <div className="project-task">
            ...
            {this.state.alltaskProject ? (
              this.state.alltaskProject.map((item, key) => (
                <div className="project-detail">
                  ...
                  <div>
                    {item.status == "complete" ?  <input type="checkbox" checked="true" className="status-checkbox mr-3" />:  <input type="checkbox" checked="true" className="status-checkbox mr-3" />}
                  ...
                  <div>
                    <Link to="/">Upload</Link>
                   {item.status =="Planned" ? <Link className="status-task" onClick={this.startModal}>Start</Link> :<p></p>} 
                   {item.status =="Process"  ? <Link className="status-task" onClick={this.startModal}>Complete</Link> :<p></p>} 
                  </div>
                </div>
              ))
            ) : (
        ...
        <Modal isOpen={this.state.isStartModal}>
          <ModalHeader>Bắt đầu công việc</ModalHeader>
          <ModalBody>
            <p>Bạn có muốn bắt đầu công việc ngay bây giờ</p>
          </ModalBody>
          <ModalFooter>
            <Button color="primary" onClick={this.startModal}>
              Save changes
            </Button>
          </ModalFooter>
        </Modal>
