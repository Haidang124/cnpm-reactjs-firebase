...
class CourseDetail extends Component {
  ...
  addTopic = async () => {
    ...
    var newTopic = {
      ...
      member: [],
      codeCourses:this.props.match.params.code,
      alltaskProject:[],
      completeTaskProject:[],
      planTaskProject:[],
      processTaskProject:[],
      fileProject :[],
      Chat:[],
      photoURL:store.getState().userProfile.firstName +
    };
    ...
    await db.collection("topics").doc(this.state.codeCourse).update(newdata);
  };
  checkJoin = (key, uid) => {
    if (
      this.state.topic[key].member.length >=
      this.state.topic[key].number + 1
    ) {
      alert("Đã quá số thành viên quy định");
      return true;
    }
    for (var i = 0; i < this.state.topic[key].member.length; i++) {
      if (this.state.topic[key].member[i].uidUser == uid) {
  ...
  removeTopic = (keyTopic, uidCreator) => {
    if (uidCreator === store.getState().userAuth.uid) {
      alert("Xóa thành công");
      this.state.mytopics.map((item, key) => {
        if (item.keyTopic == keyTopic) {
          this.state.mytopics.splice(key, 1);
          var newmydata = {
            mytopics: this.state.mytopics,
          };
           db.collection("users").doc(store.getState().userAuth.uid).update(newmydata);
  ...
  removePost = async (keyPost, uidUser) => {
    if (uidUser === store.getState().userAuth.uid) {
      alert("Xóa thành công");
        ...
        <div className="comment-post">
          ...
          {this.state.post.map((item, key) => (
            <Comment
              ...
              editPost={(keyPost, uidUser) => this.editPost(keyPost, uidUser)}
            />
          ))}
        </div>
