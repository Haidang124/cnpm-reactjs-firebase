   ...
   if(this.state.indexComment != -1)
   {
    ...
    .onSnapshot((doc) => {
      // doc.data().post[this.state.indexComment].comment.map((item, key) => {
        ...
        if (item.keyComment == this.props.keyComment) {
          this.setState({
            currentComment: doc.data().post[this.state.indexComment].comment[key],
            fullDataComment: doc.data().post[this.state.indexComment].comment,
          });
        }
      });
    });
